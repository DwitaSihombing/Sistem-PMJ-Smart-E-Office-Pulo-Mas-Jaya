                            ...
                            <tbody class="text-xs">
                                @foreach ($letters as $letter)
                                    <tr>
                                        ...
                                        <td>{{ $letter->status ?? "Belum Digenerate" }}</td>
                                        <td>
                                            <div class="table-action flex justify-center items-center gap-3">
                                                @role('director')
                                                <a href="{{ route('outgoing-mails.status', ['id' => $letter->id]) }}" class="action-icon"> <i class="mdi mdi-pen"></i></a>
                                                @endrole
                                                @unlessrole('director')
                                                <a href="{{ route('outgoing-mails.show', ['id' => $letter->id]) }}" class="action-icon"> <i class="mdi mdi-eye"></i></a>
                                                <a href="{{ route('outgoing-mails.edit', ['id' => $letter->id]) }}" class="action-icon"> <i class="mdi mdi-pen"></i></a>
